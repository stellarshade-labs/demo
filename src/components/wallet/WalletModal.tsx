...
export function WalletModal({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  useEffect(() => {
    ...
    // The overlay now genuinely covers the page, so stop the content behind it
  ...
  return (
    <Portal>
      {/*
        `items-start` + `my-auto` rather than `items-center`: a flex-centered
        child taller than its container overflows in BOTH directions, and the
        part above the top edge cannot be scrolled to. This centers normally but
        degrades to a scroll when the panel outgrows a short viewport.
      */}
      <div
        role="dialog"
        aria-modal="true"
      ...
      >
        ...
        {/* Only the list scrolls, so the header and the Recheck control stay reachable. */}
        <div className="min-h-0 flex-1 divide-y divide-ink-700 overflow-y-auto">
          {connectors.map((c) => {
            ...
            // Undetected wallets stay clickable: detection is best-effort, and
            // an extension that injected late will connect fine even though the
            // probe missed it. Trying and failing is better than a dead row.
            ...
            return (
              ...
              >
                ...
                {busy && (
                  <span className="size-1.5 animate-shade-pulse rounded-full bg-copper-500" />
                )}
              </button>
            );
          })}
