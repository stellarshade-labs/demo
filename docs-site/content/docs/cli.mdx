...
`shade` is the reference command-line tool and the fastest way to run the whole flow. It ships in the repository (it isn't published to npm), so run it from a checkout. Seven commands: `keygen`, `address`, `send`, `scan`, `balance`, `claim`, `withdraw`.
