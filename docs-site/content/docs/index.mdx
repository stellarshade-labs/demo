...
The `shade` command-line tool ships in the same repository (it isn't published to npm). Prefer to run it yourself first? Clone the repo, deploy the pool contract to Stellar testnet, and walk a full send → scan → claim cycle (needs Node.js 20+, the Stellar CLI, and a Rust toolchain):
