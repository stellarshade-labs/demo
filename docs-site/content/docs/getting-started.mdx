...
The SDK is published to npm as `stellar-shade`; the CLI and relayer run from a checkout of the [repo](https://github.com/stellarshade-labs/Shade). To use the SDK in your own application:

```bash
npm install stellar-shade
```

To build the CLI, the contract, and the rest of the workspace from a checkout:
